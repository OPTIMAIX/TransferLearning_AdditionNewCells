...
#Function that appends a new row to the previously created file with loss values for each tenant
def export_loss_results(file_name_loss_results,index,loss_tenants,k_tenants):
  new_row=[]
  new_row.append(index)
  for k in range(k_tenants):
    new_row.append(loss_tenants[k].numpy())

  #Open the file in append mode
  with open(file_name_loss_results, 'a+', newline='') as write_obj:
    csv_writer=csv.writer(write_obj)
    csv_writer.writerow(new_row)
